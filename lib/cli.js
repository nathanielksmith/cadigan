...
var cli = {
    ...
    start: function(args, cb) {
        ...
        port = args[1] || 3000
        cadigan.get_auth(function(err, auth) {
            if (err) return cb(err)
            if (auth) return start_server(hostname, port, cb)
            questions({
                username: 'username? ',
                password: 'password? ',
                site_name: 'site name? ',
            ...
            cadigan.set_auth(auth, function(err) {
                if (err) return cb(err)
                var meta = {
                    _id: 'meta',
                ...
                cadigan.set_meta(meta, function(err) {
                    if (err) return cb(err)
                    process.stdout.write('got you down as '+auth.username+'\n')
                    process.stdout.write('your site is called '+meta.site_name+'\n')
                    process.stdout.write('your name is '+meta.username)
    ...
    edit: function(args, cb) {
        ...
        var id = args[0]
        var self = this
        cadigan.get_post(id, function(err, doc) {
