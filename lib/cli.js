...
Object.defineProperty(Array.prototype, 'zipk', { value:function(b) {
...
var cli = {
    ...
    new: function(args, cb) {
        ...
        if (args.length < 1) { cb('no title provided') }
        var title = args[0]
        ...
        this.editor(filename, function(code) {
            ...
            fs.readFile(filename, function(err, data) {
                ...
                questions({tags:'tags? '}, function(answers) {
                    var tags = []
                    if (answers.tags) { tags = answers.tags.split(',') }
                    var post = {
                        title: title,
                        content: content,
                        tags: tags,
            ...
            self.editor(filename, function(code) {
                ...
                fs.readFile(filename, function(err, data) {
                    ...
                    questions(prompts, function(err,answers){
                        if (answers.tags) { doc.tags = answers.tags }
    ...
    search: function(args, cb) {
        if (args.length < 1) { throw 'need keyword' }
        cadigan.search(keyword, function(err, docs) {
            if (err) { throw err }
