...
(function() {
  var app, async, cadigan, consolidate, ensure_auth, express, hash, mkfeed, readline, update_feed, wmd;
  ...
  app.get('/api/post', function(req, res) {
    return cadigan.init(function(err) {
      return cadigan.get(req.body.post_id, function(err, doc) {
  ...
  exports.start = function(hostname, port) {
    return cadigan.init(function(err) {
      return cadigan.store.get('auth', function(err, doc) {
        var rl, start;
        start = function() {
          ...
          return console.log("cadigan listening at " + hostname + ":" + port);
        };
        if (!err) {
          return start();
        }
        ...
        return async.series({
          username: function(cb) {
            return rl.question('username? ', function(answer) {
              return cb(null, answer);
            });
          },
          password: function(cb) {
            return rl.question('password? ', function(answer) {
              return cb(null, answer);
            });
          },
          site_name: function(cb) {
            return rl.question('site name? ', function(answer) {
              return cb(null, answer);
            });
          },
          author_name: function(cb) {
            return rl.question('your name? (used to display author)', function(answer) {
              return cb(null, answer);
            });
          }
        }, function(err, input) {
          ...
          return cadigan.store.save(auth, function(err, doc) {
            var meta;
            if (err) {
              throw err;
            }
            ...
            return cadigan.store.save(meta, function(err, doc) {
