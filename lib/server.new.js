...
var route_cls = function(cls) {
    var model = new cls
    app.get(model.url(), function(req, res) {
        model.fetch({
            error: function(m,e) { res.send(404) },
            success: function(model) {
                res.send(model.toJSON())
            }
        })
    })
    app.delete(model.url(), function(req, res) {
        model.destroy({
            error: function(m,e) { res.send(404) },
            success: function(model) {
                res.send(model.toJSON())
    ...
    app.post(model.url(), function(req, res) {
    })
}
;[Post, Authdata, Metadata, Posts].forEach(route_cls)
