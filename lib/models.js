...
if (!browser) {
    var Backbone = require('backbone'),
        ...
        DOCPATH = process.env['HOME']+'/.cadigan/docstore'
}
else {
    if (!(Backbone && _)) {
        // TODO use a front-end loader
    ...
    sync: browser ?
        ...
        : function(method, model, options) {
            ...
            ds.open(DOCPATH, function(err, store) {
                ...
                // fetch a json doc from datastore
                if (method === 'read') {
                ...
                // create a new json doc in datastore
                ...
                // update an existing json doc in datastore
                else if (method === 'update') {
                    store.get(model.get('_id'), function(err, doc) {
                        if (err) {
                            if (err.code === 'ENOENT') { doc = {} }
                            else { return error_cb(err) }
                        }
...
var Authored = CadiganModel.extend({
    ...
    publish: function(opts) {
        var save = {published:true}
        if (!this.get('published_at')) {
            save.published_at = Date.now() / 1000
        }
        return this.save(save, opts)
    },
    unpublish: function(opts) {
        return this.save('published', false, opts)
    }
})

// TODO var Page = Authored.extend({ })
...
if (browser) { window.cadigan_models = models }
else { module.exports = models }
